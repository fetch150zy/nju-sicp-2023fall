...
def number_of_six(n):
    ...
    if n // 10 == 0:
        ...
        return (1 if n % 10 == 6 else 0) + number_of_six(n // 10)
...
def pingpong(n):
    ...
    def inner(m, res, f):
        if m == n:
            return res
        if m % 6 == 0 or number_of_six(m) > 0:
            return inner(m + 1, res - f, -f)
        return inner(m + 1, res + f, f)
    return inner(1, 1, 1)
...
def missing_digits(n):
    ...
    if n // 10 == 0:
        ...
        if last2 == last1 - 1 or last2 == last1:
            return missing_digits(n // 10)
        else:
            return last1 - last2 - 1 + missing_digits(n // 10)
...
def count_change(total, next_money):
    ...
    def inner(n, m):
        ...
        if m is None or n < m:
            return 0
        return inner(n - m, m) + inner(n, next_money(m))
...
number_of_six_maker = lambda f: lambda r: 0 if 0 == r else int(r % 10 == 6) + f(r // 10)
